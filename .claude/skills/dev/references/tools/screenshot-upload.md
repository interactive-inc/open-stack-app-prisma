...
保存先は `/tmp/pr-screenshots/` を使う（無ければ `mkdir -p`）。リポジトリ内（`workspace/` 含む）には置かない。プロジェクトに残す必要のない一時ファイルであり、git status を汚さない・誤コミットされないことを優先する。
...
```bash
URL=$(inta tools images upload /tmp/pr-screenshots/after.png)
